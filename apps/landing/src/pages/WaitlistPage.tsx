...
const WaitlistPage: FC = () => {
  ...
  return (
    <main className="min-h-screen bg-background-light flex flex-col relative overflow-hidden">
      ...
      <header className="w-full max-w-7xl mx-auto px-6 py-8 relative z-10 flex items-center justify-between">
        <Link to="/" className="flex items-center gap-1 group">
          <span className="font-grotesk font-bold tracking-tighter text-slate-800 text-3xl flex items-center gap-[4px] transition-transform group-hover:scale-105">
            dupi<span className="w-2.5 h-2.5 rounded-full bg-brand-orange"></span>
          </span>
        </Link>
        <Link to="/" className="text-slate-500 hover:text-brand-orange flex items-center gap-2 text-sm font-bold transition-colors">
      ...
      <div className="flex-1 flex flex-col items-center justify-center px-4 sm:px-6 relative z-10 pb-20">
        ...
        >
          ...
          <h1 className="header-font text-5xl sm:text-6xl md:text-7xl leading-[1.1] mb-6 tracking-tighter text-slate-900">
            Study Smarter, <br className="hidden sm:block" />
            <span className="serif-accent italic text-brand-orange">Not Harder.</span>
          </h1>
          
          <p className="text-lg text-slate-600 mb-12 max-w-xl mx-auto leading-relaxed">
            DUPI uses AI to instantly convert your lecture notes into interactive tests and flashcards. Join the exclusive waitlist to be instantly notified when we launch.
          </p>
          ...
          <div className="w-full max-w-md mx-auto relative">
            <AnimatePresence mode="wait">
              {status === 'success' ? (
                <motion.div
                  ...
                  className="bg-emerald-50 border border-emerald-100 p-8 rounded-3xl flex flex-col items-center justify-center gap-4 shadow-lg shadow-emerald-500/5"
                >
                  <div className="w-16 h-16 bg-emerald-100 text-emerald-500 rounded-full flex items-center justify-center mb-2">
                    <span className="material-symbols-outlined text-3xl">task_alt</span>
                  </div>
                  <h3 className="text-2xl font-bold header-font text-emerald-950">You're on the list!</h3>
                  <p className="text-emerald-700 text-sm">Keep an eye on your inbox. We'll be in touch soon.</p>
                </motion.div>
              ) : (
                <motion.form
                  key="form"
                ...
                >
                  ...
                  <div className="absolute -inset-2 rounded-full pointer-events-none">
                    <svg className="absolute inset-0 w-full h-full overflow-visible" xmlns="http://www.w3.org/2000/svg">
                      <rect
                        width="100%" height="100%" rx="36" fill="none"
                  ...
                  <div className="relative flex items-center bg-white rounded-full p-2 shadow-sm border border-slate-100 z-10 w-full overflow-hidden transition-shadow group-focus-within:shadow-md">
                    ...
                    <input
                      ...
                      className="flex-1 bg-transparent border-none outline-none text-slate-700 placeholder:text-slate-400 font-medium py-3 h-11 w-full"
                    />
                    <Button
                      type="submit"
                      disabled={status === 'loading'}
            ...
            <motion.p 
              className="text-xs text-slate-400 mt-6 font-medium flex items-center justify-center gap-2"
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
