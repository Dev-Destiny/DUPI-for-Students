...
export const generateTest = async (documentId: string, userId: string, numQuestions: number = 10, difficulty: string = "medium", topic: string = "", questionType: string = "mcq") => {
  const document = await prisma.document.findFirst({
    where: { id: documentId, userId }
  });

  if (!document || !document.processed) {
    throw new ApiError(400, "DOC_NOT_READY", "Document is not processed yet.");
  }

  const collectionId = document.chromaCollectionId;
  if (!collectionId) {
    throw new ApiError(400, "CHROMA_NOT_FOUND", "No vector collection found for this document.");
  }

  // Request generation from Python service
  const response = await axios.post(`${env.AI_PROCESSOR_URL}/generate/test`, null, {
    params: {
...
export const generateFlashcards = async (documentId: string, userId: string, numCards: number = 15, difficulty: string = "medium", topic: string = "") => {
  const document = await prisma.document.findFirst({
    where: { id: documentId, userId }
  });
  ...
  // Request generation from Python service
  const response = await axios.post(`${env.AI_PROCESSOR_URL}/generate/flashcards`, null, {
    params: {
      collection_id: collectionId,
      num_cards: numCards,
      difficulty: difficulty,
...
export const generateSummary = async (documentId: string, userId: string) => {
  ...
  const response = await axios.post(`${env.AI_PROCESSOR_URL}/generate/summary`, null, {
