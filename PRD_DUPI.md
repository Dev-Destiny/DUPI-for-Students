...
### Design System & Aesthetics (Midnight Academic Theme)

- **Theme Identity:** The application follows a "Midnight Academic" aesthetic, defaulting to a dark-mode-first experience (`#09090B`) that feels premium, scholarly, and modern, paired with a strict semantic token system.
- **Fonts**:
    - Significant Headers: **Lora** (maintains academic styling and serif contrast).
    - Body Text & Minor Headers: **Space Grotesk** (techy, modern, legible vibe).
- **Core Semantic Colors**:
    - Primary Background: `#09090B` (`bg-background`) serves as the dark mode base for the web app, dashboard, and onboarding.
    - Elevated Cards/Surfaces: Structured using semantic tokens like `bg-card` and `bg-muted` rather than hardcoded colors.
    - Brand Orange: `#FF6F20` (`bg-brand-orange`) for vibrant accents, shortcuts, and primary actions.
    - Brand Violet: Deep Purple `#62109F` (`bg-brand-violet`) for structural backgrounds, active states, and premium CTAs.
    - Brand Gold: `#FFD54F` (`bg-brand-gold`) for highlighting "Pro" access limits and monetization upsells.
- **UI Element Highlights**:
    - Widespread adoption of semantic Tailwind variables (`text-foreground`, `border-border`) ensures perfect adaptable contrast.
    - **Animations**: `framer-motion` is strictly required for animations, replacing standard CSS transitions for complex interactions.
    - Glassmorphic blurs and multi-layered gradient orbs reinforce the dark academic ambiance.

### Architecture & Modularity Philosophy
...
│   │   │   │   │   ├── pages/
│   │   │   │   │   └── views/  # Separated screen components
│   │   │   │   └── home/       # Authenticated app domain
│   │   │   │       ├── dashboard/
│   │   │   │       ├── documents/
│   │   │   │       └── layouts/
│   │   │   ├── components/     # Shared global components
│   │   │   │   └── ui/         # Base configurations
│   │   │   ├── hooks/
...
_This PRD will evolve through development cycles, user feedback, and market changes. All stakeholders should review updates and provide input during iteration planning._
